...
class Simulation:
    ...
    def __init__(self):
        ...
        self.reproduction_cost = 1.2

        # objects
        self.foods = [(random.uniform(0, self.field_size), random.uniform(0, self.field_size)) for i in range(self.starting_food)]
    ...
    def move_frame(self, dt):
        ...
        if random.uniform(0, 1) < self.food_spawn_probability:
            self.foods.append((random.uniform(0, self.field_size), random.uniform(0, self.field_size)))
